...
- Landing → player uses a two-phase solid-color curtain: cover the viewport, commit the layout under the opaque curtain, then reveal the persistent stage and staggered console. The interstitial leads with the editorial phrase “The room is listening.” rather than repeating the product name. Never snapshot or duplicate the Pixi canvas; use transform and opacity only, and honor reduced motion.
- Focus mode is a reversible staged shot. Entering withdraws header context and the score strip, slides the focus control in from the edge, moves the camera controls, then pushes Hiyori's Pixi camera 0.2× closer. Exiting reverses the camera and brings the desk, score, and controls back with staggered motion. Never restore focus chrome as a one-frame visibility cut.
